...
namespace QuantConnect.Algorithm.CSharp
{
    public class IndexOptionCallButterflyAlgorithm : QCAlgorithm
    {
        ...
        public override void OnData(Slice slice)
        {
            ...
            // Byt the call butterfly
            var callButterfly = OptionStrategies.CallButterfly(_spxw, sortedCalls[^1].Strike, atmCall.Strike, sortedCalls[0].Strike, expiry);
            var price = callButterfly.OptionLegs.Sum(x => Math.Abs(Securities[x.Symbol].Price * x.Quantity) * _multiplier);
            if (price > 0)
            {
