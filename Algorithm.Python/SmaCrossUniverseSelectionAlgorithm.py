...
class SmaCrossUniverseSelectionAlgorithm(QCAlgorithm):
    ...
    def initialize(self):
        ...
        ibm = self.add_equity("IBM", Resolution.tick).symbol
        ibmSma = self.SMA(ibm, 40)
        self.log(f"{ibmSma.name}: {ibmSma.current.time} - {ibmSma}. IsReady? {ibmSma.is_ready}")
        ...
        self.log(f"SPY    - Is ready? SMA: {spySma.is_ready}, ATR: {spyAtr.is_ready}, VWAP: {spyVwap.is_ready}")
        ...
        self.log(f"EURUSD - Is ready? SMA: {eurSma.is_ready}, ATR: {eurAtr.is_ready}")
