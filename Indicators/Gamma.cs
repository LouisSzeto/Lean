        ...
        protected override decimal CalculateGreek(DateTime time)
        {
            ...
            switch (_optionModel)
            {
                case OptionPricingModelType.BinomialCoxRossRubinstein:
                    ...
                    var fU = OptionGreekIndicatorsHelper.CRRTheoreticalPrice(ImpliedVolatility, sU, Strike, timeToExpiration, RiskFreeRate, DividendYield, Right);
                    var fM = OptionGreekIndicatorsHelper.CRRTheoreticalPrice(ImpliedVolatility, UnderlyingPrice, Strike, timeToExpiration, RiskFreeRate, DividendYield, Right);
                    var fD = OptionGreekIndicatorsHelper.CRRTheoreticalPrice(ImpliedVolatility, sD, Strike, timeToExpiration, RiskFreeRate, DividendYield, Right);

                    var deltaU = (fU - fM) / (sU - UnderlyingPrice);
                    var deltaD = (fM - fD) / (UnderlyingPrice - sD);
                ...
                default:
                    ...
                    // allow at least 1% IV
                    return math(norm.Density, -d1) * math(Math.Exp, -DividendYield * timeToExpiration) / 
                        UnderlyingPrice / Math.Max(ImpliedVolatility, 0.01m) / math(Math.Sqrt, timeToExpiration);
            }
        }
    }
}
