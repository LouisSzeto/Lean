        ...
        protected override decimal CalculateGreek(DateTime time)
        {
            ...
            switch (_optionModel)
            {
                ...
                case OptionPricingModelType.ForwardTree:
                    ...
                    {
                        var upFactor = math(Math.Exp, (RiskFreeRate - DividendYield) * deltaTime + ImpliedVolatility * math(Math.Sqrt, deltaTime));
                        if (upFactor == 1m)
                        {
                            // Introduce a very small factor to avoid constant tree while staying low volatility
                            upFactor = 1.0001m;
                        }
                        var downFactor = math(Math.Exp, (RiskFreeRate - DividendYield) * deltaTime - ImpliedVolatility * math(Math.Sqrt, deltaTime));
                        if (downFactor == 1m)
                        {
                            // Introduce a very small factor to avoid constant tree while staying low volatility
                            downFactor = 0.9999m;
