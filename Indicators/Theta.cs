        ...
        protected override decimal CalculateGreek(DateTime time)
        {
            ...
            switch (_optionModel)
            {
                case OptionPricingModelType.BinomialCoxRossRubinstein:
                    var thetaTime = timeToExpiration / 200;

                    var forwardPrice = OptionGreekIndicatorsHelper.CRRTheoreticalPrice(ImpliedVolatility, UnderlyingPrice, Strike, timeToExpiration - 2 * thetaTime, RiskFreeRate, DividendYield, Right);
                    ...
                    return (forwardPrice - price) * 0.5m / thetaTime / 365m;
