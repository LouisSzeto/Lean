    ...
    public class ThetaTests : OptionBaseIndicatorTests<Theta>
    {
        ...
        public void ComparesAgainstExternalData(string subPath, bool reset, double errorMargin, bool singleContract, int callColumn = 15, int putColumn = 14)
        {
            ...
            foreach (var line in File.ReadAllLines(path).Skip(3))
            {
                ...
                {
                    callIndicator = new Theta(call, interestRate, dividendYield, model);
                    putIndicator = new Theta(put, interestRate, dividendYield, model);
                }
                else
                {
                    callIndicator = new Theta(call, put, interestRate, dividendYield, model);
                    putIndicator = new Theta(put, call, interestRate, dividendYield, model);
                }
        ...
        // No American option theta from QuantLib
        public void ComparesAgainstExternalData2(decimal price, decimal spotPrice, OptionRight right, int expiry, double refTheta, OptionStyle style)
        {
            ...
            var indicator = new Theta(symbol, 0.0403m, 0.0m, model, OptionPricingModelType.BlackScholes);
